...
type User struct {
	...
	Password_hash string        `gorm:"size:128" json:"password_hash,omitempty"` //用户密码加密的
	...
	Avatar_url    string        `gorm:"size:256" json:"avatar_url"`              //用户头像路径       通过Kodo对象存储进行图片存储
...
type Area struct {
	...
	Houses []*House `json:"houses,omitempty"`     //区域所有的房屋   与房屋表进行关联
}
...
func SetupMysql() {
	...
	if err != nil {
		log.Fatal("读取Mysql配置时出错，错误为: ", err)
	}
	...
	if err != nil {
		log.Println("打开数据库时失败, error=" + err.Error())
		return
	}
	sqlDB, err := DB().DB()
	if err != nil {
		log.Println("获取Mysql变量失败, error=" + err.Error())
		return
	}
